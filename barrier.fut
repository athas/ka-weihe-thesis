...
let outer [n][m] (xs: [n]f64) (ys: [m]f64): *[n][m]f64 =
  map (\x -> map (\y -> x * y) ys) xs 

let matsub [n][m] (xss: [n][m]f64) (yss: [n][m]f64): *[n][m]f64 =
  map2 (map2 (-)) xss yss

let identity (n: i64): [n][n]f64 = 
  tabulate_2d n n (\i j -> if j == i then 1f64 else 0f64)

let vecdiv_scalar [n] (xs: [n]f64) (k: f64): *[n]f64 =
  map (/k) xs

let forward_substitution [n] (L: [n][n]f64) (b: [n]f64): [n]f64 =
...
let lu [n] (A: [n][n]f64) : ([n][n]f64, [n][n]f64) =
  let L = identity n
  let (L, A) = loop (L, A) = (L, copy A) for k in 0..<(n) do
...
let solveLUb [n] (L: [n][n]f64) (U: [n][n]f64) (b: [n]f64) =
  forward_substitution L b |> back_substitution U 

let lu_solveAB [n][m] (A: [n][n]f64) (B: [m][n]f64) =
  let (L, U) = lu A
  in map (solveLUb L U) B |> transpose

let lu_inv [n] (A: [n][n]f64): [n][n]f64 =
  lu_solveAB A (identity n)

let eye (n: i64): [n][n]f64 =
  let arr = map (\x ->
    let i = x / n
    let j = x % n
    in f64.bool (i == j)
...
let h [n] (w: [n]f64) (mu: [n]f64) (esg: [n]f64) =
...
let phi [n] (w: [n]f64) (mu: [n]f64) (esg: [n]f64) =
...
let f_hess_auto [n] (w: [n]f64) (S: [n][n]f64) (mu: [n]f64) (esg: [n]f64) (t: f64) =
    hess (\w -> f w S mu esg t) w

-- let f_hessian_inv [n] (w: [n]f64) (S: [n][n]f64) (mu: [n]f64) (esg: [n]f64) (t: f64) =
--     lu_inv (f_hess w S mu esg t)

let f_hessian_inv_auto [n] (w: [n]f64) (S: [n][n]f64) (mu: [n]f64) (esg: [n]f64) (t: f64) =
    lu_inv (f_hess_auto w S mu esg t)

let argmax_f (x:f64,i:i64) (y:f64,j:i64) : (f64,i64) =
  if x > y then (x, i)
...
-- TODO: solve for w0
let find_feasible [n] (mu: [n]f64) (esg: [n]f64) =
...
let newton [n] (w: [n]f64) (f: [n]f64 -> f64 -> f64) (f_grad: [n]f64 -> f64 -> [n]f64) (f_hessian_inv: [n]f64 -> f64 -> [n][n]f64) (t: f64) (beta: f64) (alpha: f64) (max_iter: i64) (eps: f64) =
  let (w, i, lambd, r) = loop (w, i, lambd, r) = (w, 0, 0.0, true) while i < max_iter && r do
    let fw = f w t
    ...
    in if lambd / 2.0 <= eps then (w, i+1, lambd, false)
    else
      let (new_w, new_f, s, running) = loop (new_w, new_f, s, running) = (w, fw, 1.0, true) while running do
        let new_w = map2 (+) w (map (*s) v)
        let new_f = f new_w t
        in if new_f <= fw - alpha * s * lambd then (new_w, new_f, s, false)
        else (new_w, new_f, s * beta, true)
      in if fw == new_f then (w, i+1, lambd, false)
...
let barrier [n] (w: [n]f64) (f: [n]f64 -> f64 -> f64) (f_grad: [n]f64 -> f64 -> [n]f64) (f_hessian_inv: [n]f64 -> f64 -> [n][n]f64) (beta: f64) (alpha: f64) (max_iter: i64) (eps: f64) (m: f64) =
    let (w, gap, iters, obj, t, r) = loop (w, gap, iters, obj, t, r) = (w, [], [], [], 1f64, true) while r do
        if (f64.i64 n) / t <= eps then (w, gap, iters, obj, t*m, false)
        else
          let (w, i, lambd) = newton w f f_grad f_hessian_inv t beta alpha max_iter eps
          let gap = gap ++ [n / 1]
          let iters = iters ++ [i]
...
let w0 = find_feasible mu esg_scores

let f' = (\w t -> f w S mu esg_scores t)
let f_grad_auto' = (\w t -> f_grad_auto w S mu esg_scores t)
let f_hess_auto' = (\w t -> f_hess_auto w S mu esg_scores t)
let f_hessian_inv_auto' = (\w t -> f_hessian_inv_auto w S mu esg_scores t)
