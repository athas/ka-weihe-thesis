...
def tril_indices n : *[](i16, i16) =
  let s = tri_num n
  let is = iota s
  let is = map (\x -> tril x n) is
  in is

def low_outer (v: []f64) (ti: [](i16, i16)) : *[]f64 =
  map (\x ->
          let (i, j) = x
          in #[unsafe] v[i] * v[j])
      ti
...
def i_j_to_index_colwise_diag i j (n: i64) : i64 =
  let p = n - j - 1
...
def admm [n] [m] -- objective function
         (f: [n]f64 -> f64)
