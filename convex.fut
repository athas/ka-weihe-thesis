...
module mk_solver (T: real) : convex with t = T.t = {
  ...
  def power_iteration [n] [m] (A: [n][m]t) (max_iter: i64) (tol: t) : ([n]t, t, [m]t) =
    ...
    let (v, _, _) =
      loop (v, i, r) = (v, 0, true) while r && i < max_iter do
        let old_v = v
  ...
  def line_search [n]
                  ...
                  (max_iter: i64) : t = -- Maximum number of iterations
    let (t, _) =
      loop (t, i) = (T.f64 1.0, 0) while i <= max_iter && !(armijo f x delta_x grad alpha t) do
        (t T.* beta, i + 1)
    in t
  ...
  def gradient_descent [n]
    ...
    let (x, _, _) =
      loop (x, i, r) = (x0, 0, true) while i < max_iter && r do
  ...
  def newtons_method [n]
                     ...
                     (max_iter: i64) : [n]t =
    let res =
      loop (x, i, r) = (x0, 0, true) while r && i < max_iter do
        let grad = f_grad x
        let hess = f_hess x
  ...
  def newton_equality [n] [m]
    ...
    let res =
      loop (x, i, r) = (x0, 0, true) while r && i < max_iter do
        ...
        in if linalg.vecnorm delta_x T.< epsilon
           then (x, i + 1, false)
           else (x_new, i + 1, true)
    in res.0
  ...
  def newton_equality_ls [n] [m]
    ...
    let f_hess = hess_jvp f
    let (x, _, _) =
      loop (x, r, i) = (x0, true, 0) while r && i < max_iter do
        let grad_val = f_grad x
        let hessian_val = f_hess x
        let KKT = linalg.matzeros (n + m) (n + m)
        let rhs = linalg.veczeros (n + m)
  ...
  def barrier_method [n] [m]
    ...
    let res =
      loop (x, t) = (x0, t0) for i < iters do
        ...
        let delta_x = delta_x_lam[:n]
        let s =
          loop s = (T.f64 1.0) while T.isnan (f (map2 (\a b -> a T.+ b T.* s) x delta_x)) && s T.> T.f64 1e-15 do
            s T.* (T.f64 0.8)
  ...
  -- ADMM
  -- Ax >= b
  def admm [n] [m] -- gradient of the objective function
           (f_grad: [n]t -> [n]t)
           ...
           (max_iter: i64) : [n]t = -- maximum number of iterations (default: 1000)
    ...
    let r = true
    let res =
      loop (x, z, u, r, i) while r && i < max_iter do
        let x_prev = x
        let x = update_x x z u
        let z = update_z x u
        let u = update_u x z u
        in if convergence_criterion x x_prev
           then (x, z, u, false, i + 1)
           else (x, z, u, true, i + 1)
    in res.0
}
