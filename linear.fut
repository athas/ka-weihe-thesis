...
module mk_linear (T: real) : linear with t = T.t = {
  ...
  def seq_lud [n] (A: [n][n]t) : ([n][n]t, [n][n]t) =
    ...
    in loop (L, U) for i in 0..<n do
         let (L, U) =
           loop (L, U) for j in (i + 1)..<n do
             let Lji = copy (U[j, i] T./ U[i, i])
             let L[j, i] = Lji
             let (L, U) =
               loop (L, U) for k in i..<n do
  ...
  def cholesky_banachiewicz [n] (A: [n][n]t) : [n][n]t =
    ...
    in loop L for i in 0..<n do
         loop L for j in 0...i do
           let sum = seq_dotprod L[i, :j] L[j, :j]
           in if i == j
              then let L[i, j] = T.sqrt (copy (A[i, i] T.- sum))
                   in L
              else let L[i, j] = (copy ((T.f64 1.0) T./ L[j, j]) T.* (copy (A[i, j] T.- sum)))
                   in L
  ...
  def cholesky_outer [n] (A: [n][n]t) : [n][n]t =
    let A =
      loop A = copy A for j in 0..<n do
        let m = n - j - 1
  ...
  def solve_Ab_gauss [m] (A: [m][m]t) (b: [m]t) =
    b |> sized (m * 1) |> unflatten |> solve_AB_gauss A |> flatten |> sized m
  ...
  def solve_Ab_gauss_np [m] (A: [m][m]t) (b: [m]t) =
    b |> sized (m * 1) |> unflatten |> gauss_np_solveAB A |> flatten |> sized m
  ...
  def solve_Ab_cg [n] (A: [n][n]t) (b: [n]t) (x0: [n]t) (tol: t) (max_iter: i64) =
    ...
    let running = true
    let res =
      loop (x, p, r, rs_old, i, running) while running && i < max_iter do
        let i = i + 1
        ...
        in if T.sqrt rs_new T.< tol
           then (x, p, r, rs_old, i, false)
           else let p = map2 (T.+) r (map ((rs_new T./ rs_old) T.*) p)
