...
let cholesky_banachiewicz [n] (A: [n][n]f64): [n][n]f64 =
  let L = replicate n (replicate n 0.0)
  in loop L for i in 0..<n do
    loop L for j in 0...i do
      let sum = loop sum = 0.0 for k in 0..<j do
        L[i,k] * L[j,k] + sum
      in if i == j then
        let L[i,j] = f64.sqrt (A[i,i] - sum)
        in L
      else
        let L[i,j] = (1.0 / L[j,j] * (A[i,j] - sum))
        in L

let cholesky_crout [n] (A: [n][n]f64): [n][n]f64 =
  let L = replicate n (replicate n 0.0)
...
let cho [n] (A: *[n][n]f64): [n][n]f64 = 
  tril <| loop A for j in 0..<n do
    let k = n - j - 1
