...
def all_close2d [n] [m] (xss: [n][m]f64) (yss: [n][m]f64) (eps: f64) : bool =
...
entry test_newton_equality =
  ...
  let b_eq = [0.0]
  let x0 = [-1.0, 1.0]
  let x_min = mod.newton_equality_ls f A b_eq x0 1e-8 0.3 0.8 10000 100000
...
def range1 = (range 0.0 0.072 1000)
...
def main () =
  ...
  let b = [1.0]
  let res = map (\r -> mod.solve_qp_auto (h r) f0 A b) range1
  in res
